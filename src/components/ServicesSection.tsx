...
const serviceConfigs = [
  { icon: Globe, slug: "website-creation", accent: "175 70% 40%" },
  { icon: Search, slug: "seo", accent: "45 90% 55%" },
  { icon: ShoppingCart, slug: "e-commerce", accent: "280 60% 55%" },
  { icon: MessageSquare, slug: "consulting", accent: "210 80% 55%" },
  { icon: Palette, slug: "web-design", accent: "350 75% 55%" },
  { icon: Wrench, slug: "support-maintenance", accent: "150 60% 40%" },
];

const ServiceCard = ({ service, index }: { service: typeof serviceConfigs[0]; index: number }) => {
    ...
    >
      ...
      >
        ...
        <div className="relative z-10">
          ...
          >
            <service.icon size={22} strokeWidth={1.5} style={{ color: `hsl(${service.accent})` }} />
          </div>
          <h3 className="text-lg font-medium mb-3 tracking-tight text-foreground/90 group-hover:text-foreground transition-colors">
            {t(`services.items.${service.slug}.title`)}
          </h3>
          <p className="text-muted-foreground text-sm leading-relaxed font-light group-hover:text-foreground/65 transition-colors">
